...
var PushNotificationService = class _PushNotificationService {
  ...
  showNotification(title, options) {
    ...
    const notification = new Notification(title, __spreadValues({
  ...
  static \u0275fac = function PushNotificationService_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _PushNotificationService)(\u0275\u0275inject(Router));
  };
...
// node_modules/@capawesome/capacitor-badge/dist/esm/index.js
var Badge = registerPlugin("Badge", {
  web: () => import("./chunk-GH6UGHUJ.js").then((m) => new m.BadgeWeb())
});
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(BadgeService, [{
    type: Injectable,
    args: [{
      providedIn: "root"
    }]
...
var NotificationService = class _NotificationService {
  ...
  badgeService;
  apiUrl = ApiConfig.ENDPOINTS.NOTIFICATIONS;
  unreadCountSubject = new BehaviorSubject(0);
  unreadCount$ = this.unreadCountSubject.asObservable();
  messageUnreadCountSubject = new BehaviorSubject(0);
  ...
  chatMessageReceivedSubject = new Subject();
  chatMessageReceived$ = this.chatMessageReceivedSubject.asObservable();
  messageReadSubject = new Subject();
  messageRead$ = this.messageReadSubject.asObservable();
  typingSubject = new Subject();
  typing$ = this.typingSubject.asObservable();
  groupMessageReceivedSubject = new Subject();
  groupMessageReceived$ = this.groupMessageReceivedSubject.asObservable();
  constructor(http, router, pushNotificationService, badgeService) {
    this.http = http;
    this.router = router;
  ...
  getAllNotifications() {
    return this.http.get(`${this.apiUrl}?includeRead=true`);
  }
  getUnreadNotifications() {
    return this.http.get(`${this.apiUrl}/Unread`).pipe(catchError(() => of([])));
  }
  loadUnreadCount() {
    this.http.get(`${this.apiUrl}/Count`).pipe(catchError(() => of({ count: 0 }))).subscribe((response) => this.unreadCountSubject.next(response.count));
  }
  markAsRead(id) {
    return this.http.post(`${this.apiUrl}/MarkAsRead/${id}`, {}).pipe(catchError(() => of({})), tap(() => this.loadUnreadCount()));
  }
  markAllAsRead() {
    return this.http.post(`${this.apiUrl}/MarkAllAsRead`, {}).pipe(catchError(() => of({})), tap(() => this.unreadCountSubject.next(0)));
  }
  deleteNotification(id) {
    return this.http.delete(`${this.apiUrl}/${id}`).pipe(tap(() => this.loadUnreadCount()));
  }
  ...
  startConnection(userId, token) {
    ...
    this.hubConnection.on("ReceiveMessageCount", (count) => {
      this.messageUnreadCountSubject.next(count);
    });
    this.hubConnection.on("ReceiveNewMessage", (data) => {
    ...
    this.hubConnection.on("ReceiveMessagesRead", (data) => {
      console.log("[SignalR] ReceiveMessagesRead", data);
      this.messageReadSubject.next(data);
    });
    this.hubConnection.on("ReceiveTyping", (data) => {
    ...
    }).then(() => console.log("[SignalR] Joined group User_" + userId)).catch((err) => console.error("[SignalR] connection error:", err));
  }
  stopConnection() {
  ...
  static \u0275fac = function NotificationService_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _NotificationService)(\u0275\u0275inject(HttpClient), \u0275\u0275inject(Router), \u0275\u0275inject(PushNotificationService), \u0275\u0275inject(BadgeService));
  };
...
(() => {
  ...
  }], () => [{ type: HttpClient }, { type: Router }, { type: PushNotificationService }, { type: BadgeService }], null);
})();
...
//# sourceMappingURL=chunk-PPPAYIKG.js.map
